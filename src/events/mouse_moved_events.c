...
{
    if ((pos_mouse.x > edit_dropdown->origin.x && pos_mouse.x < (edit_dropdown->origin.x + edit_dropdown->size.x)) && (pos_mouse.y > edit_dropdown->origin.y && pos_mouse.y < (edit_dropdown->origin.y + edit_dropdown->size.y))) {
        edit_dropdown->button_state = HOVER;
        sfRectangleShape_setFillColor(edit_dropdown->sprite, (sfColor) {250, 20, 102, 100});
    } else {
        edit_dropdown->button_state = NONE;
        sfRectangleShape_setFillColor(edit_dropdown->sprite, (sfColor) {200, 200, 200, 100});
    }
}
